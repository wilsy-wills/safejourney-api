<?php
    function send_message(pdo $connection) {
        ...
        $statement = $connection->prepare("INSERT INTO tj_message(message, sender_id, receiver_id, request_id, date_created)
        ...
        if ($_POST['user_cat'] == "user_app") {
            $statement = $connection->prepare("SELECT fcm_id AS fcm_token FROM driver_info WHERE driver_id = :driver_id LIMIT 1");
        ...
        if ($fcm_token != "") {
            $statement = $connection->prepare("SELECT message_id, message, request_id, sender_id, receiver_id, date_created
                    FROM tj_message WHERE message_id = :message_id LIMIT 1");
