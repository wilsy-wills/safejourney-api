<?php
    function save_rating(pdo $connection) {
        try {
            $statement = $connection->prepare("SELECT rating_id, rating, request_id, message, date_created, date_modified 
                        FROM driver_rating WHERE request_id = :request_id LIMIT 1");
            $statement->bindParam(':request_id', $_POST['request_id']);
            $statement->execute();
            $rating_data = $statement->fetch();

            if ($rating_data) {
                $statement = $connection->prepare("UPDATE driver_rating SET rating = :rating, date_modified = :current_date,
                         message = :message WHERE rating_id = :rating_id");
                $statement->bindParam(':', $rating_data['rating_id']);
            } else {
                $statement = $connection->prepare("INSERT INTO driver_rating (request_id, rating, message, date_created, date_modified, driver_id) 
            ...
            $statement = $connection->prepare("SELECT sum(rating) AS ratings, count(rating_id) AS total 
                    FROM driver_rating WHERE driver_id = :driver_id LIMIT 1");
            $statement->bindParam(':driver_id', $_POST['driver_id']);
            $statement->execute();
            ...
            $statement = $connection->prepare("UPDATE driver_info SET total_raters = :total_raters, total_rating = :total_rating 
